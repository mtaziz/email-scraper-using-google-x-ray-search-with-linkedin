...
# from .spiders_settings import namespider1_settings
# from ..namespider1_crawlspider_settings import custom_settings
...
try:
    if os.path.exists("import/namespider1_{0}_items.csv".format(google_search_keywords_input)):
        os.remove(
...
class GoogleNameSpider1(CrawlSpider):
    # timestamp = datetime.now().strftime('%Y-%b-%d')
    timestamp = datetime.now().strftime('%Y-%b-%d_%H%M')

    # name = 'google_spider'
    name = 'namespider1_crawlspider'
    allowed_domains = ['google.com', 'linkedin.com']
    # Ignore the HTTP status code
    handle_httpstatus_list = [404, 302, 301, 429, 503]
    # custom_settings = custom_settings
    custom_settings = {
        'GOOGLE_SEARCH_KEWORDS': google_search_keywords_input,
        'GOOGLE_SEARCH_KEWORDS_WITH_OPERATORS': google_search_keywords_with_operators_detail,
        'CRAWLERA_ENABLED': True,
        'CRAWLERA_USER': CRAWLERA_USER_API_KEY,
        'CRALERA_PRESERVE_DELAY': True,
        'DOWNLOAD_DELAY': 15,
        'FEED_URI': "import/namespider1_{0}_items.csv".format(google_search_keywords_input),
    ...
    # How to run the spider: scrapy crawl namespider1 start_urls=""
    ...
    #     # self.custom_settings['GOOGLE_SEARCH_KEWORDS_WITH_OPERATORS']
    #
    #     for gs_result_start in range(0, GS_RESULTS_PAGES_NUM):
    #         link = 'https://www.google.com/search?num={0}&start={1}&hl=en&q={2}'.format(GS_RESULTS_NUM, gs_result_start, start_urls_form_sanitized)
    #         if " " in link:
    #             link = link.replace(' ', '+')
    #             yield Request(
    #                             url=link,
    ...
    start_urls = ['https://www.google.com/search?client=ubuntu&channel=fs&q=site%3Alinkedin.com+erythrocyte+red+gene%28phD+OR+PH.D+D.Phil%29+viral+lines+-jobs&ie=utf-8&oe=utf-8']

    # red blood cell : Test
    start_urls = [
        'https://www.google.com/search?client=ubuntu&channel=fs&q=Red+blood+cell%22+-jobs+-topic+site%3Alinkedin.com&ie=utf-8&oe=utf-8']
    ...
    # Make sure "logs directory exists if it does not exist then create it on the go"
    # try:
    #     os.makedirs(LOGGING_DIR)
    # except OSError as e:
    #     if e.errno != errno.EEXIST:
